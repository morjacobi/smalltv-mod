...
void clockReapply(const Settings& s) {
  ...
  // time whether or not night mode is on. (A reboot loop was traced to this
  // device's power supply, not to SNTP running continuously — see
  // wifi-weather-smalltv-mod memory for the isolation sequence: the loop
  // persisted even with the clock feature compiled out entirely.)
